...
def test_opencode_go_uses_opencode_credentials() -> None:
    access = resolve_model_connection(
        "opencode-go/model",
        ModelConnectionSpec(passthrough=True),
        _env({"OPENCODE_API_KEY": "opencode-key"}),
    )

    assert access.provider == "opencode"
    assert access.api_key == "opencode-key"
    assert access.env == {"OPENCODE_API_KEY": "opencode-key"}
